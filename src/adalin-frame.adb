...
is
   ...
   procedure Calculate_FID_Parity (F : in out Frame) is
      ...
      function Bit_Of (V : Bits6; N : Natural) return Boolean with Pre => N < 6 is
      begin
         return (V and Bits6 (2 ** N)) /= 0;
      end Bit_Of;

   begin
      P0 := Bit_Of (ID, 0) xor Bit_Of (ID, 1) xor Bit_Of (ID, 2) xor Bit_Of (ID, 4);
      P1 := not (Bit_Of (ID, 1) xor Bit_Of (ID, 3) xor Bit_Of (ID, 4) xor Bit_Of (ID, 5));
   ...
   function Calculate_Data_Checksum (F : Frame; mode : Mode_Type) return Byte is
      sum : Integer := 0;
   begin
      if mode = Enhanced and then (not (F.frame_identifier in 16#3C# .. 16#3D#)) then
   ...
   procedure SetData (F : in out Frame; New_Data : Data_Array; mode : Mode_Type) is
   begin
